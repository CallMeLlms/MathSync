...
// ─── OptionButton ───
const OptionButton = ({ label, index, isSelected, disabled, onPress }) => {
  ...
  const colors = isSelected
    ? { border: Colors.secondary, bg: Colors.secondaryContainer, text: Colors.onSecondaryContainer }
    : { border: Colors.outlineVariant, bg: Colors.surfaceContainerLowest, text: Colors.onSurface };

  return (
    <Animated.View entering={ZoomIn.springify().delay(index * 80)} style={styles.optionWrapper}>
      <Pressable
...
// ─── ExamVisualPickerEngine ───
const ExamVisualPickerEngine = ({ data, onResult }) => {
  ...
  const handleCheck = () => {
    ...
    setResolved(true);
    onResult(isCorrect, [String(selectedOption)]);
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <CheckButton
        onPress={handleCheck}
        disabled={!selectedOption || resolved}
        label={resolved ? 'ANSWERED ✓' : 'CHECK'}
      />
    </View>
  );
};
...
export default ExamVisualPickerEngine;
